  ...
  return (
    ...
    >
      ...
      <Button
        ...
        className="h-[68px] flex justify-start gap-[18px] text-2xl ssp-font"
      >
        <LogoutIcon />
        Logout now
      </Button>
...
interface ISidebarButton {
  ...
  path: string;
  closeSidebar: (path?: string) => void;
}

const SidebarButton = ({ children, path, closeSidebar }: ISidebarButton) => {
  return (
    <Button
      onClick={() => closeSidebar(path)}
      asChild
      variant="ghost"
      className="h-[68px] flex justify-start gap-[18px] text-2xl ssp-font px-4 py-2"
    >
      <Link href={path}>
